@if ($paginator->hasPages())
    <nav class="d-flex justify-items-center justify-content-between w-100">
        ...
        {{-- MOBİL GÖRÜNÜM (Sadece Önceki/Sonraki Butonları) --}}
        ...
        <div class="d-none flex-sm-fill d-sm-flex align-items-sm-center justify-content-sm-between">
            ...
            {{-- SOL TARAF: BİLGİLENDİRME METNİ --}}
            <div>
                <p class="small text-muted m-0">
                    @if (App::getLocale() == 'tr')
                        {{-- Türkçe Cümle Yapısı --}}
                        Toplam <span class="fw-semibold">{{ $paginator->total() }}</span> sonuçtan
                        <span class="fw-semibold">{{ $paginator->firstItem() }}</span> -
                        <span class="fw-semibold">{{ $paginator->lastItem() }}</span> arası gösteriliyor
                    @else
