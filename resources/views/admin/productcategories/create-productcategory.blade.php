...
@section('content')
    <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
        <h6 class="mb-0 text-uppercase">Ürün Kategori Ekle</h6>
    </div>
    <hr/>
    <div class="card">
        <div class="card-body p-4">
            <form action="{{ route('admin.productcategories.index') }}" class="row g-3" method="POST" enctype="multipart/form-data">
                @csrf
                ...
                <div class="col-md-12">
                    ...
                    <select name="category_id" id="category_id" class="single-select-field form-select">
                        ...
                        @foreach ($productcategories as $productcategory)
                            <option value="{{ $productcategory->id }}" {{ old('category_id') ? 'selected' : '' }}>{{ $productcategory->name_tr }} - {{ $productcategory->name_en }}</option>
                        @endforeach
                    </select>
