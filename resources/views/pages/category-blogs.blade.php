...
@section('content')
    ...
    <main class="mb-5 pb-5">
        <div class="container">
            ...
            {{-- first blog --}}
            @if(!empty($category) && !empty($category->blogs))
                @php $firstBlog = $category->blogs->first(); @endphp
                @if($firstBlog)
            ...
            {{-- blogs card --}}
            <div class="row">
                @forelse ($category->blogs as $key => $blog)
                    @if($key != 0)
                                ...
                                <div class="d-flex flex-wrap gap-2 z-index-1 position-absolute top-0 start-0 px-4 pt-3">
                                    @if($blog->categories)
                                        @foreach ($blog->categories as $category)
                                            <button class="btn btn-secondary btn-sm">{{ App::getLocale() == 'tr' ? $category->name_tr : $category->name_en }}</button>
                                        @endforeach
                                    @endif
                                </div>
